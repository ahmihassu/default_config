SELECT 
COUNT(DISTINCT person.person_id) AS "Total number of patients with hypoxemia (SPO2 <90)"
FROM person
...
LEFT JOIN obs form_under_obs1 ON encounter.encounter_id = form_under_obs1.encounter_id
                        AND form_under_obs1.value_numeric < 90
                        AND form_under_obs1.concept_id IN (SELECT concept_id from concept_name WHERE concept_name_type = "FULLY_SPECIFIED" AND name LIKE "SPO2 Numeric")
