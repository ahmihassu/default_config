...
COUNT(DISTINCT CASE WHEN TIMESTAMPDIFF(YEAR, person.birthdate, obs.date_created) > 65 AND person.gender = 'M' THEN person.person_id END) AS "> 65_Male",
COUNT(DISTINCT CASE WHEN TIMESTAMPDIFF(YEAR, person.birthdate, obs.date_created) > 65 AND person.gender = 'F' THEN person.person_id END) AS "> 65_FEmale" 
FROM person
...
JOIN location ON location.location_id = obs.location_id
