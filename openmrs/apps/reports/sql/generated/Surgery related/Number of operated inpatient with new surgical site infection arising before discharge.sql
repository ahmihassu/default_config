...
(SELECT name FROM concept_name WHERE concept_id = form_under_obs2.value_coded AND concept_name_type = "FULLY_SPECIFIED") AS "Department"
FROM person
...
LEFT JOIN obs form_under_obs2 ON encounter.encounter_id = form_under_obs2.encounter_id
                        AND form_under_obs2.concept_id IN (SELECT concept_id from concept_name WHERE concept_name_type = "FULLY_SPECIFIED" AND name LIKE "Department, operative notes")
WHERE (form_under_obs1.obs_id IS NOT NULL AND form_under_obs0.obs_id IS NOT NULL)
GROUP BY form_under_obs2.value_coded
