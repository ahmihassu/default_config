SELECT 
COUNT(DISTINCT person.person_id) AS "Number of operated inpatients discharged alive (including transfers out)"
FROM person
...
LEFT JOIN obs form_under_obs2 ON encounter.encounter_id = form_under_obs2.encounter_id
                        AND form_under_obs2.concept_id IN (SELECT concept_id from concept_name WHERE concept_name_type = "FULLY_SPECIFIED" AND name LIKE "Operative Notes")
WHERE (form_under_obs2.obs_id IS NOT NULL AND (form_under_obs0.obs_id IS NOT NULL OR form_under_obs1.obs_id IS NOT NULL))
