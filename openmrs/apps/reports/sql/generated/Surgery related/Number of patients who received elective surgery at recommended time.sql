SELECT 
COUNT(DISTINCT person.person_id) AS "Number of patients who received elective surgery at recommended time"
FROM person
...
JOIN surgical_appointment sa ON sa.patient_id = encounter.patient_id AND date(sa.actual_end_datetime) = date(form_under_obs0.date_created)